...
it is not a network-isolation switch. The real export smoke test observed a
MediaPipe Clearcut upload attempt that failed under network restrictions.
Assess dependency telemetry and enforce network policy at deployment level
if fully offline operation is required.

Enrollment images, JSONL events, logs and annotated media can contain personal
information. The application has no built-in authentication, encryption or
retention policy; protect the local directories and configure deletion and
access rules before use. Only use trusted model/checkpoint and manifest files.
